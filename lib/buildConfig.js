        ...
        for (var i = buildList.length - 1; i >= 0; i--) {
            ...
            * */
            if(_self._type == 'img') {
                ...
                */
                config[baseName + i].dest = buildList[i].replace(/[a-zA-Z][\w-]*$/, function (matching) {
                    return matching + typeMap[_self._type]['postfix'];
                });
                config[baseName + i].destCss = buildList[i].replace(/\/images\/([a-zA-Z][\w-]*)$/, function (matching, g1) {
                    return '/sass/sprites/_sprite_' + g1 + '.scss';
                });
                config[baseName + i].imgPath = '../images/' + baseName + typeMap[_self._type]['postfix'];
            }
        }
